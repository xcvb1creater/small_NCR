...
from utils import AverageMeter, ProgressMeter, load_checkpoint
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, captions, lengths, ids) in enumerate(data_loader):
        ...
        # cache embeddings (ids is a tuple; use array indexing to avoid numpy multi-dim bug)
        batch_ids = np.asarray(ids, dtype=int)
        img_embs[batch_ids] = img_emb.data.cpu().numpy().copy()
        cap_embs[batch_ids, : max(lengths), :] = cap_emb.data.cpu().numpy().copy()
